...
추출한 용도 후보 목록** 을 번호로 제시하고 LLM 에게 그중 하나를 고르게 한다
(자유 텍스트 생성이 아니라 '번호 선택').

설계 핵심:
    * 2-Field 분리: LLM 은 텍스트가 아니라 번호(`selection`)를 반환한다. 목록에 없으면
      `selection=0` 을 반환하며, 이때만 `suggested_code`/`suggested_name` 을 자유 제안한다.
    * 등록된 룰 한정: LLM 은 그 테넌트에 **등록된 규칙에서 추출한 후보 안에서만** 고른다.
      활성 규칙이 0개이면 애초에 이 단계까지 오지 않고 상위 그래프가 NONE(관리자 문의)로
      종료한다. (전역 기본값 콜드스타트 자동 주입은 폐지 -- 룰 없이 추측하지 않는다.)
...
# 참고용 전역 기본 용도 8종(샘플 카탈로그).
# [정책] 등록된 룰이 없을 때 이 목록을 콜드스타트로 자동 주입하지 않는다 -- 룰 없이는
# 추천하지 않고 '관리자 문의'로 안내한다. (테스트/도구에서 샘플 후보로만 참조한다.)
